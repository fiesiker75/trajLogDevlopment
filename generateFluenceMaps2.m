function [actualFluence,plannedFluence,CPindex,x,y] = generateFluenceMaps2(snapShotData, scaleFactor, subBeams)
...
% points
% abs(CP-round(CP))
NumSubBeams = length(subBeams.subBeams);
...
%get MUs per CP
MU_E = cat(1,snapShotData(CPindex).MU_E);
MU_CP_E = diff(MU_E);
MU_A = cat(1,snapShotData(CPindex).MU_A);
MU_CP_A = diff(MU_A);

for j = 1:1%NumSubBeams
    subBeams.beamCP(j).CPindex = find(CPindex>=subBeams.beamCP(j).On & CPindex<=subBeams.beamCP(j).Off);

    % CPList2 = cat(1,subBeams.beamCP.CPindex);
    ...
    for i=1:21%LastCP
        ...
        [fieldE,fieldA]=trajFluence(snapShotData(CPindex(subBeams.beamCP(j).CPindex(i))),fieldWidth,fieldHeight,X,Y);
    ...
    JawX1_E = min(-10*cat(1,snapShotData(CPindex(subBeams.beamCP(j).CPindex)).X1_E));
    JawX2_E = max(10*cat(1,snapShotData(CPindex(subBeams.beamCP(j).CPindex)).X2_E));
    JawY1_E = min(-10*cat(1,snapShotData(CPindex(subBeams.beamCP(j).CPindex)).Y1_E));
    JawY2_E = max(10*cat(1,snapShotData(CPindex(subBeams.beamCP(j).CPindex)).Y2_E));
